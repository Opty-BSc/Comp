...
Node *compareNode(Node *p, Node *n, int full) {
    ...
    else if (p->type == nodeStr && strcmp(n->value.s, p->value.s) != 0) return n;
    else if (p->type == nodeInt && n->value.i != p->value.i) return n;
    else if (p->type == nodeReal && n->value.r != p->value.r) return n;
    else if (p->type == nodeData && n->value.d.size != p->value.d.size &&
        memcmp(n->value.d.data, p->value.d.data, n->value.d.size) != 0) return n;
...
Node *copyNode(Node *p) {
    ...
    /* allocate node */
    if (p->type == nodeOpr) size += p->value.sub.num * sizeof(Node*);
...
static int print(Node *p, FILE *fp, char *tab[], int lev) {
    ...
    if (tab != 0) fprintf(fp," %s:", tab[p->attrib]);
    else fprintf(fp," %d:", p->attrib);
    switch (p->type) {
        case nodeNil: break;
...
void visitNode(Node *p, int mode, void (*func[])(Node*)) {
    unsigned i;
    if (p == 0) return;
    if (mode & PRE && func[p->attrib]) func[p->attrib](p);
    if (p->type == nodeOpr)
        for (i = 0; i < p->value.sub.num; i++)
            visitNode(p->value.sub.n[i], mode, func);
    if (mode == POS && func[p->attrib]) func[p->attrib](p);
}

void listenNode(Node *p, int mode) {
    unsigned i;
    if (p == 0) return;
    if (mode & PRE && p->listen) p->listen(p);
    if (p->type == nodeOpr)
    for (i = 0; i < p->value.sub.num; i++)
        listenNode(p->value.sub.n[i], mode);
    if (mode == POS && p->listen) p->listen(p);
}
...
void pathNode(Node *p, FILE *fp, char *tab[], char *base) {
    ...
    if (p == 0) return;
    if (fp == 0) fp = stdout;
    if (tab != 0) comp = tab[p->attrib];
    else sprintf(code,"%d", p->attrib);
